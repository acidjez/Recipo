  ...
  methods: {
    loadRecipe() {
      getDoc(doc(db, "recipes", this.id))
        .then((recipeDoc) => {
          this.recipeName = recipeDoc.data().name;
          this.recipeIngredients = recipeDoc.data().ingredients;
          this.recipeInstructions = recipeDoc.data().instructions;
          this.authorID = recipeDoc.data().userID;
          this.imageURL = recipeDoc.data().imageURL;
        })
        .catch((error) => {
    ...
    addRecipe() {
      if (this.authorID !== useLoginStore.userID) {
        alert("You are not the author of this recipe.");
        return;
      }
      if (
        !(
        ...
        this.loading = true;
        updateDoc(doc(db, "recipes", this.id), {
          ...
          .then(() => {
            if (this.imageChanged) {
              return this.fileToBucket(this.id);
            }
          })
          .then(() => {
            if (this.imageChanged) {
          ...
          .then((url) => {
            if (this.imageChanged) {
              return updateDoc(doc(db, "recipes", this.id), {
                imageURL: url,
              });
            }
          })
    ...
    fileToBucket(id) {
      if (this.recipeImage != "") {
        ...
        return deleteObject(imagesRef)
          .then(() => {
            return uploadBytes(imagesRef, this.recipeImage).then(() => {});
          })
